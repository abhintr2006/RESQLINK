...
export async function authenticateCronRequest(request: Request): Promise<Response | null> {
  const currentSecret = process.env["RESQLINK_CRON_SECRET"] || process.env["CRON_SECRET"];
  const previousSecret = process.env["RESQLINK_CRON_SECRET_PREVIOUS"] || process.env["CRON_SECRET_PREVIOUS"];
